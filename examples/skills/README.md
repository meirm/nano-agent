...
Skills are directories containing `SKILL.md` files stored in `~/.nano-cli/skills/` that:
...
nano-cli run "Generate API tests for this endpoint"
# → Automatically uses api-testing skill

nano-cli run "Analyze this sales data and create visualizations"
# → Automatically uses data-analysis skill

nano-cli run "Perform a security audit on this codebase"
...
nano-cli skills show api-testing --full
...
1. **Create directory**: `mkdir ~/.nano-cli/skills/my-skill`
...
```bash
# Copy a single skill
cp -r examples/skills/api-testing ~/.nano-cli/skills/
...
1. **Global**: `~/.nano-cli/skills/` (available everywhere)
2. **Project**: `.nano-cli/skills/` (project-specific, overrides global)
