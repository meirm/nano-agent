...
Agent files define specialized personalities that extend the base system prompt. They are stored in `~/.nano-cli/agents/` as markdown files.
...
Command files are reusable prompt templates stored in `~/.nano-cli/commands/`. They use YAML frontmatter for metadata and support `$ARGUMENTS` substitution.
...
```bash
# Execute a command
nano-cli /code-review "$(cat src/main.py)"
nano-cli /test-generator "$(cat utils/helper.py)"
nano-cli /documentation "Generate README for this project"
```

## Skills

Skills are modular capabilities that extend nano-agent functionality. They are stored in `~/.nano-cli/skills/` as directories containing `SKILL.md` files.
...
# Show skill details
nano-cli skills show api-testing
...
### Agents
Copy agent files to `~/.nano-cli/agents/`:

```bash
cp examples/agents/*.md ~/.nano-cli/agents/
```

### Commands
Copy command files to `~/.nano-cli/commands/`:

```bash
cp examples/commands/*.md ~/.nano-cli/commands/
```

### Skills
Copy skill directories to `~/.nano-cli/skills/`:

```bash
cp -r examples/skills/* ~/.nano-cli/skills/
```
...
1. **Edit the files** directly in your `.nano-cli` directory
