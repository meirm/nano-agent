...
Commands are markdown files stored in `~/.nano-cli/commands/` that:
...
nano-cli /test-generator "def add(a, b): return a + b"

# Command with multiple lines
nano-cli /documentation "$(cat - <<EOF
class MyClass:
    def method(self):
        pass
EOF
...
**Usage**: `nano-cli /code-review "$(cat src/main.py)"`
...
**Usage**: `nano-cli /test-generator "$(cat utils/helper.py)"`
...
**Usage**: `nano-cli /documentation "$(cat api/endpoints.py)"`
...
```bash
nano-cli /my-command "task description"
```
...
```bash
# Example 1
nano-cli /my-command "example input"

# Example 2
nano-cli /my-command "$(cat input.txt)"
```
...
cp examples/commands/example-code-review.md ~/.nano-cli/commands/code-review.md
...
nano-cli commands show code-review

# Create new command template
nano-cli commands create my-new-command

# Edit a command (opens in default editor)
nano-cli commands edit code-review
```
...
- User commands in `examples/dot.nano-cli/commands/`
