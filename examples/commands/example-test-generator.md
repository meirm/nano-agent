...
nano-cli /test-generator "$(cat src/calculator.py)"

# Generate tests with specific framework
nano-cli /test-generator "$(cat api/endpoints.py)" --model gpt-5
```
...
nano-cli /test-generator "$(cat models/user.py)"

# Generate tests for an entire module
nano-cli /test-generator "$(cat utils/validator.py)"
```
