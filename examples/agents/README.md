...
Agents are markdown files stored in `~/.nano-cli/agents/` that define:
...
nano-cli interactive --agent analyst
```
...
```bash
# Copy a single agent
cp examples/agents/example-coder.md ~/.nano-cli/agents/coder.md
...
- Built-in agents in `examples/dot.nano-cli/agents/`
