...
1. nano-cli loads from ~/.nano-cli/config.json (global)
2. nano-agent loads from ~/.nano-agent/config.json (global)
3. Both merge with project .nano-agent/config.json (project-specific)
...
# from nano_agent.modules.config_loader import ConfigLoader, load_configuration
...
class TestConfigurationHierarchy:
    ...
    def setup_method(self):
        ...
        # Create nano-cli and nano-agent config directories
        self.nano_cli_dir = self.home_dir / ".nano-cli"
        self.nano_agent_dir = self.home_dir / ".nano-agent"
        self.project_config_dir = self.project_dir / ".nano-agent"
    ...
    def sample_nano_cli_config(self):
        """Sample nano-cli global configuration."""
        return {
    ...
    def sample_project_config(self):
        ...
        return {
            ...
            "search_settings": {
                ...
                "indexed_extensions": [".py", ".js", ".ts", ".md", ".json", ".yaml", ".yml"]
            },
            "project_settings": {
                "name": "test-project",
                "version": "1.0.0",
    ...
    def test_nano_cli_global_config_loading(self, sample_nano_cli_config):
        """Test loading nano-cli global configuration from ~/.nano-cli/config.json."""
        # Create nano-cli config file
        ...
        with patch('pathlib.Path.home', return_value=self.home_dir):
            ...
            assert loaded_config["commands_directory"] == "~/.nano-cli/commands"
    
    def test_nano_agent_global_config_loading(self, sample_nano_agent_config):
        """Test loading nano-agent global configuration from ~/.nano-agent/config.json."""
    ...
    def test_project_config_loading(self, sample_project_config):
        """Test loading project-specific configuration from .nano-agent/config.json."""
    ...
    def test_config_merging_hierarchy(self, sample_nano_cli_config, sample_nano_agent_config, sample_project_config):
        ...
        expected_nano_cli_merge = {
            ...
            # From project
            "disabled_tools": ["delete_file", "run_terminal"],
            "commands_directory": ".nano-cli/commands"  # Overridden by project
        }
    ...
    def test_missing_config_files_handling(self):
        ...
        with patch('pathlib.Path.home', return_value=self.home_dir), \
             patch('pathlib.Path.cwd', return_value=self.project_dir):
...
class TestCommandFileCascadeLoading:
    ...
    def setup_method(self):
        ...
        self.global_commands_dir = self.home_dir / ".nano-cli" / "commands"
        self.project_commands_dir = self.project_dir / ".nano-cli" / "commands"
    ...
    def test_global_command_loading(self):
        """Test loading commands from global ~/.nano-cli/commands/ directory."""
...
"""
    ...
    def test_project_command_loading(self):
        """Test loading commands from project .nano-cli/commands/ directory."""
...
class TestConfigurationIntegration:
    ...
    def test_cli_integration_nano_cli(self):
        """Test CLI integration for nano-cli binary."""
        # Test that nano-cli binary loads:
        # 1. ~/.nano-cli/config.json (global)
        # 2. .nano-agent/config.json (project, merged)
        # 3. Commands from ~/.nano-cli/commands/*.md and .nano-cli/commands/*.md
        pass
    
    def test_cli_integration_nano_agent(self):
        """Test CLI integration for nano-agent binary.""" 
        # Test that nano-agent binary loads:
        # 1. ~/.nano-agent/config.json (global)
        # 2. .nano-agent/config.json (project, merged)
        # 3. Commands from ~/.nano-cli/commands/*.md and .nano-cli/commands/*.md
        pass
