...
import pytest
from nano_agent.mcp_tools import (get_available_models,
...
class TestMCPTools:
    ...
    async def test_list_provider_models_with_capability_filter(self):
        ...
        result = await list_provider_models(provider="anthropic", capability="vision")
    ...
    async def test_list_provider_models_deprecated_filter(self):
        ...
        result_with_deprecated = await list_provider_models(
            provider="anthropic", include_deprecated=True
        )
        ...
        assert len(result_with_deprecated["models"]) >= len(
            result_no_deprecated["models"]
        )
