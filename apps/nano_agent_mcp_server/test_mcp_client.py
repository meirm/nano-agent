...
async def test_mcp_server():
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            print(f"Server name: {init_result.server_name if hasattr(init_result, 'server_name') else 'Unknown'}")
            print(f"Protocol version: {init_result.protocol_version if hasattr(init_result, 'protocol_version') else 'Unknown'}")
            print("\n" + "="*60 + "\n")
