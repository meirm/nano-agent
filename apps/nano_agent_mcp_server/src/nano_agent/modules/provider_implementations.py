...
from .model_provider_config import ModelProviderConfigManager
...
class OpenAIProvider(ModelProvider):
    ...
    async def list_models(self, **kwargs) -> List[ModelInfo]:
        ...
        if not self.api_key:
            raise ProviderAuthenticationError("OpenAI API key not configured", provider="openai")
                    ...
                    for model_data in data.get("data", []):
                        ...
                        if any(x in model_id for x in ["gpt", "davinci", "turbo"]):
                            model = ModelInfo(
                                ...
                                name=model_id,
                                provider="openai",
                                created_at=datetime.fromtimestamp(model_data.get("created", 0))
                            )
    ...
    async def list_models(self, **kwargs) -> List[ModelInfo]:
        ...
        for url in urls_to_try:
            try:
                ...
                last_error = ProviderConnectionError(
                    "Ollama server request timed out",
                    provider="ollama"
                )
...
class LMStudioProvider(ModelProvider):
    ...
    async def list_models(self, **kwargs) -> List[ModelInfo]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=10) as response:
                    ...
                    # Alternative format with models key
                        ...
                        for model_data in data["models"]:
                            model_id = model_data.get("id") or model_data.get("name", "unknown")
                            model = ModelInfo(
                                id=model_id,
            ...
            raise ProviderConnectionError(
                f"LMStudio server not running at {self.base_url}",
                provider="lmstudio"
            )
        except asyncio.TimeoutError:
            raise ProviderConnectionError(
                "LMStudio server request timed out",
                provider="lmstudio"
            )
