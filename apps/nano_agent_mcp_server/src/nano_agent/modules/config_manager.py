"""
Configuration Manager for Nano Agent

Provides hierarchical configuration loading with the following precedence:
1. Command-line arguments (highest priority)
2. Environment variables
3. Project-level config (.nano-agent.yaml in current directory)
4. User-level config (~/.config/nano-agent/config.yaml)
5. System-level config (/etc/nano-agent/config.yaml)
...
class ConfigManager:
    ...
    def __init__(
        self, config_path: Optional[Path] = None, app_name: str = "nano-agent"
    ):
        """
    ...
    def load_config(self) -> NanoAgentConfig:
        ...
        if (
            self.app_name == "nano-cli"
            and self.user_config_path
    ...
    def _load_env_config(self) -> Dict[str, Any]:
        ...
        Environment variables should be prefixed with NANO_AGENT_
        Examples:
            NANO_AGENT_DEFAULT_PROVIDER=ollama
            NANO_AGENT_DEFAULT_MODEL=gpt-oss:120b
            NANO_AGENT_LOG_LEVEL=DEBUG
        ...
        for key, value in os.environ.items():
            if key.startswith(prefix):
                ...
                # Handle nested keys (e.g., NANO_AGENT_PROVIDER_OLLAMA_API_BASE)
                if "_" in config_key:
    ...
    def save_project_config(self, config: Optional[NanoAgentConfig] = None) -> None:
        ...
        # Use first project config name
        config_path = Path.cwd() / self.PROJECT_CONFIG_NAMES[0]
