...
class ProviderConfig:
    ...
    def get_model_settings(self, model: str, provider: str, base_settings: dict) -> ModelSettings:
        ...
        if not provider_config:
            logger.warning(f"No configuration found for provider {provider}, using base settings")
        ...
        if model.startswith("gpt-5"):
            ...
            if "max_tokens" in filtered_settings:
                filtered_settings["max_completion_tokens"] = filtered_settings.pop("max_tokens")
    ...
    def create_agent(
        ...
        if not provider_config:
            ...
            if provider not in ["openai", "anthropic", "ollama", "lmstudio", "ollama-native"]:
        ...
        if provider == "openai":
            ...
            if api_base:
                ...
                ollama_url = provider_config.api_base if provider_config else "http://localhost:11434/v1"
            ...
            if api_base:
                ...
                ollama_host = provider_config.api_base if provider_config else "http://localhost:11434"
            ...
            if provider_config and provider_config.api_base:
                logger.info(f"Creating agent for custom provider {provider} with model: {model}")
            ...
            if provider == "ollama" or provider == "ollama-native":
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        discovered_models = [m["name"] for m in models_data.get("models", [])]
                        logger.info(f"Discovered {len(discovered_models)} models from Ollama")
    ...
    def validate_model_provider_combination(
        ...
        if not provider_config:
            ...
            logger.warning(f"No configuration for provider {provider}, allowing model {model}")
            return True, None
    ...
    def validate_provider_setup(self, provider: str, model: str) -> tuple[bool, Optional[str]]:
        ...
        is_valid, error = self.validate_model_provider_combination(provider, model, strict=False)
        if not is_valid:
            return False, error
        ...
        if provider in ["ollama", "ollama-native"]:
            try:
                ...
                ollama_url = provider_config.api_base if provider_config else "http://localhost:11434"
                # Remove /v1 suffix if present for API endpoint
                api_url = ollama_url.rstrip('/').removesuffix('/v1')
            ...
            try:
                lmstudio_url = provider_config.api_base if provider_config else "http://localhost:1234"
                if not lmstudio_url.endswith("/v1"):
                    lmstudio_url = f"{lmstudio_url.rstrip('/')}/v1"
                ...
                if response.status_code == 200:
                    ...
                    if model not in models and (not provider_config or not provider_config.allow_unknown_models):
...
def create_agent(
    ...
    return get_provider_config().create_agent(
        name, instructions, tools, model, provider, 
        model_settings, api_base, api_key
    )
...
def validate_model_provider_combination(
    ...
    """Backward compatibility wrapper."""
    return get_provider_config().validate_model_provider_combination(provider, model, strict)