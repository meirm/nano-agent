...
This module handles loading and executing user-defined tools from ~/.nano-cli/tools/.
...
def get_tools_directory() -> Path:
    ...
    return Path.home() / ".nano-cli" / "tools"


def get_allowed_tools() -> Optional[list]:
    """Get the list of allowed tools from ~/.nano-cli/allowed-tools.json if it exists."""
    allowlist_path = Path.home() / ".nano-cli" / "allowed-tools.json"
