        ...
        if not config_manager.is_model_allowed(provider, model):
            ...
            if not provider_config.allow_unknown_models:
                ...
                if len(provider_config.known_models) > 5:
                    models_str += f", ... and {len(provider_config.known_models) - 5} more"
                return False, f"Model {model} not available for {provider}. Available models: {models_str}"
            else:
                # Unknown models are allowed, but warn the user
                logger.info(f"Model {model} not in known models for {provider}, but provider allows unknown models")
        
        # Now call the original validation for API key and service checks
        return OriginalProviderConfig.validate_provider_setup(provider, model, available_models, provider_requirements)
...
def patch_constants():
    ...
    """
    import nano_agent.modules.constants as constants
...
def enable_flexible_configuration():
    ...
    import nano_agent.modules.provider_config as provider_module
