...
class SkillLoader:
    ...
    - Global: ~/.nano-cli/skills/
    - Project: .nano-cli/skills/ (overrides global)
    ...
    def __init__(self, working_dir: Optional[Path] = None):
        ...
        if not self.builtin_skills_dir.exists():
            ...
            try:
                ...
                try:
                    # Python 3.9+
                    with pkg_resources.path("nano_agent.data.builtin_skills", "") as path:
                        self.builtin_skills_dir = Path(path)
                except (ImportError, TypeError, AttributeError):
                    # Fallback: try direct import
                    try:
                        from nano_agent.data import builtin_skills
